#!/usr/bin/env python
import roslib; roslib.load_manifest('atlas_trajectory_control')
...
if __name__ == "__main__":
	...
	if len(sys.argv) != 3:
		print "usage: traj.py YAML_FILE TRAJECTORY_NAME"
	...
	rospy.Subscriber("/atlas/joint_states", JointState, JointStateCallback)
	...
	# get gain from parameter server
	rospy.init_node('atlas_trajecotry_control')
	...
	for i in xrange(0, traj_len):
		...
		intialPosition = array(currentJointState.position)
		...
		for ratio in linspace(0, 1, n):
			interpCommand = (1 - ratio) * intialPosition + ratio * commandPosition
