...
class Switch:
	...
	def demo(self):
		...
		stand_prep_msg.position = [2.438504816382192e-05, 0.0015186156379058957, 9.983908967114985e-06, -0.0010675729718059301,
		 -0.0003740221436601132, 0.06201673671603203, -0.2333149015903473, 0.5181407332420349, -0.27610817551612854,
		 -0.062101610004901886, 0.00035181696875952184, -0.06218484416604042, -0.2332201600074768, 0.51811283826828, 
		...
		# set k_effort = [255] to indicate that we still want all 
		# joints under user control.
		# The stand behavior need a few iterations before it start
		...
		# Step 3: Move the arms and head a little 
		slight_movement_msg = AtlasCommand()
		# Always insert current time
		slight_movement_msg.header.stamp = rospy.Time.now()
		# Start with 0.0 and set values for the joints that
		# we want to control
		...
		slight_movement_msg.kp_position = [20.0, 4000.0, 2000.0, 20.0, 
		5.0, 100.0, 2000.0, 1000.0, 900.0, 300.0, 5.0, 100.0, 2000.0, 
		1000.0, 900.0, 300.0, 2000.0, 1000.0, 200.0, 200.0, 50.0, 100.0,
		2000.0, 1000.0, 200.0, 200.0, 50.0, 100.0]
		slight_movement_msg.ki_position = [0.0] * self.NUM_JOINTS
		slight_movement_msg.kd_position = [0.0] * self.NUM_JOINTS
		# Bump up kp_velocity to reduce the jerkines of the transition
		...
		# set k_effort = [255] for the joints that we want to control
		...
		for i in range(4):
			...
			# We will specify desired foot poses in ego-centric frame
			...
			# current estimated pose in the world
			# which is combination of IMU and internal position estimation
			f1 = posemath.fromMsg(step_data.pose)
			f2 = PyKDL.Frame(PyKDL.Rotation.Quaternion(self.imu_msg.orientation.x,
				self.imu_msg.orientation.y, self.imu_msg.orientation.z,
				self.imu_msg.orientation.w), PyKDL.Vector(self.asis_msg.pos_est.position.x,
				self.asis_msg.pos_est.position.y, self.asis_msg.pos_est.position.z))

			f = f2 * f1
			step_data.pose = posemath.toMsg(f)
			walk_msg.walk_params.step_queue[i] = step_data

		# Use the same k_effort from the last step, to retain user control
		# over some joints. BDI has control of the other joints
		...
		home_msg.kp_position = [20.0, 4000.0, 2000.0, 20.0, 5.0, 100.0,
		 2000.0, 1000.0, 900.0, 300.0, 5.0, 100.0, 2000.0, 1000.0, 900.0,
		 300.0, 2000.0, 1000.0, 200.0, 200.0, 50.0, 100.0, 2000.0, 1000.0,
		...
		# set k_effort = [255] to indicate that we want all joints
		# under user control
