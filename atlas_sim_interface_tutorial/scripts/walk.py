...
class AtlasWalk():
	...
	def walk(self):
		...
		self.mode = rospy.Publisher('/atlas/mode', String, None, False, True, None, queue_size=1)
	...
	# /atlas/atlas_state callback. This message provides the orientation 
	# of the robot from the torso IMU
	# This will be important if you need to transform your step commands from
	# the robot's local frame to world frame
	...
	def dynamic(self, state):
		...
		# A walk behavior command needs to know three additional steps beyond the 
		# current step needed to plan
		...
		for i in range(4):
			...
			# Swing height to be implemented
		...
		# Publish this command every time, new state message 
		self.asi_command.publish(command)
		# End of dynamic walk behavior

	# commanding a static walk/step behavior
	def static(self, state):
		# When the robot status_flags are 1 (swaying), publish the next step command
		...
		# Determince pose of the next step based on the number of steps we have taken
		command.step_params.desired_step.pose = self.calculate_pose(self.step_index)

		# Publish a nex step command every time a state message is received
		self.asi_command.publish(command)
		# End of static walk behavior

	# method to calculate a pose of step based on the step_index 
	...
	def calculate_pose(self, step_index):
		...
		# There will be 60 steps to a circle, 
		# position along the cirlce is current step
		...
		# will be place onto the ground if the ground is lower than z
		pose.position.z = 0
